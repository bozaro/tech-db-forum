...
func init() {
	Register(Checker{
		...
		Description: "",
		FnCheck:     CheckForumGetThreadsSimple,
		Deps: []string{
	...
	Register(Checker{
		...
		Description: "",
		FnCheck:     CheckForumGetThreadsNotFound,
		Deps: []string{
			"thread_create_simple",
		},
	})
...
func CheckForumGetThreadsSimple(c *client.Forum) {
	pass := 0
	for true {
		pass++
...
func CheckForumGetThreadsNotFound(c *client.Forum) {
	forum := RandomForum()
	pass := 0
	for true {
		pass++
